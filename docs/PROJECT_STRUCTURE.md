...
├── artifacts/                    # Everything training.py produces — generated, not hand-written
│   ├── model/                      # Trained model (SavedModel format)
│   ├── class_labels.json
│   ├── ood.npz
...
- `artifacts/` is entirely generated by `training.py` — model weights, labels, OOD
  reference embeddings, and evaluation reports live together, separate from
  hand-written source. Nothing under `src/`, `api/`, or `web/` writes here.
